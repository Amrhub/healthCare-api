class Api::V1::FriendshipsController < ApplicationController
  ...
  def create
    ...
    if @friendship.save
      render json: { **format_friendship_json(@friendship), userInfo: user_info }, status: :created
    else
  ...
  def update
    if @friendship.update(friendship_params)
      render json: format_friendship_json(@friendship)
    else
      render json: @friendship.errors, status: :unprocessable_entity
    end
  end
  ...
  def show_friendships
    ...
    @friendships.each do |friendship|
      ...
      when 'pending'
        data = {
          **format_friendship_json(friendship),
  ...
  def format_friendship_json(friendship)
    {
      id: friendship.id,
